...
2. 点击按钮会展开非阻塞式操作弹窗（**支持随时点击「取消」或点击外部直接关闭，绝不锁死输入框**）：
   - 点击 **「✅ 开启后续自动联网搜索」**：当前会话正式进入联网搜索模式，按钮变为高亮的 `[ 🌐 联网搜索 · 已开启 ✕ ]`，后续对话中的所有提问默认强制调用网页搜索；
   - 点击 **`✕`** 按钮：可随时一键取消并退出联网搜索模式；
   - 若输入框已有文本，还会出现 **「⚡ 立即单次检索当前输入内容」** 选项，点击即刻单次查阅。

### 方式 2：斜杠命令触发
在对话输入框中直接输入：
```text
/search 你的搜索关键词      # 单次直接检索
/search on                  # 开启当前会话的持续联网搜索模式
/search off                 # 关闭当前会话的联网搜索模式
```
